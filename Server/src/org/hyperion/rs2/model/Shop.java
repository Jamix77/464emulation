...
public class Shop {
	...
	/*public static void restock() {
		...
		World.getWorld().submit(new Tickable(random){
			@Override
			public void execute() {
				try {
					Shop.init();
					System.out.println("tick check" +random);
...
//		XMLController.writeXML(shops, file);
	 ...
	 */
	public static final int SHOP_INVENTORY_INTERFACE = 300;
	...
	public static void open(Player player, int shopId, int stockType) {
		...
		switch (stockType) {
		case 1: // Main stock
			player.getInterfaceState().addListener(shop.getMainStock(), new InterfaceContainerListener(player, SHOP_INVENTORY_INTERFACE, 75, 93));
	...
	public static void buyItem(Player player, int slot, int id, int amount) {
		Shop shop = shops.get(player.getInterfaceState().getOpenShop());
		Item item = shop.getMainStock().get(slot);
			...
			if(shop.getMainStock().get(slot).getCount() > 0) {
				...
				if(!shop.hasCurrency(player, item, amount)) {
					...
					if(shop.getDefaultStock().contains(item.getId())) { //If the player stock started with this item, it goes to 0, rather than removing completely.
						shop.getMainStock().removeOrZero(new Item(item.getId(), amount));
					} else {
						shop.getMainStock().remove(new Item(item.getId(), amount));
					}
				}
			}
			break;
	...
	public int getSellValue(Player player, Item item) {
		...
		switch(getShopType()) {
		case GENERAL_STORE:
			if(shop.getMainStock().getCount(item.getId()) > 260) {
				return (int) (ItemDefinition.forId(item.getId()).getValue() * 0.3 < 1 ? 1 : ItemDefinition.forId(item.getId()).getValue() * 0.3);
			} else {
	...
	public static void costItem(Player player, int slot, int id) {
		Shop shop = shops.get(player.getInterfaceState().getOpenShop());
		Item item = player.getInterfaceState().getOpenStockType() == 1 ? shop.getMainStock().get(slot) : shop.getMainStock().get(slot);
		if(item == null || item.getId() != id) {
			return; // invalid packet, or client out of sync
		}
	...
	public static void valueItem(Player player, int slot, int id) {
		...
		for(int i = 0; i < ItemDefinition.untradableItems.length; i++) {
			if(item.getId() == ItemDefinition.untradableItems[i] || item.getId() == 995) {
				player.getActionSender().sendMessage("You cannot sell this item.");
				return;
			}
		}
