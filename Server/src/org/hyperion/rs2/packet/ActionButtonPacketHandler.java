...
import org.hyperion.rs2.model.Animation.Emote;
...
import org.hyperion.rs2.model.Shop.ShopType;
import org.hyperion.rs2.model.Skills.SkillCape;
...
public class ActionButtonPacketHandler implements PacketHandler {
	...
	public void handle(final Player player, Packet packet) {
		...
		case 14:
			...
			switch(button) {
				case 130:
					Bank.open(player, BankScreenType.BANK_PIN_CONFIRM);
					break;
				case 148:
					player.getActionSender().sendString(14, 177, "7 days");
				case 230:
					Bank.open(player, BankScreenType.BANK_PIN_SETTINGS);
					break;
				case 225:
					Bank.open(player, BankScreenType.BANK_PIN_ENTER);
					break;
			}
			break;
		case 13:
			switch(button) {
				...
				case 108:
				case 109:
					Bank.clickNumber(player, button);
					break;
			}
			break;
		/*
			...
			case 4:
				...
				case MODERN_MAGICS:
					...
					if(player.getEquipment().get(Equipment.SLOT_WEAPON).getId() == 4170) {
						player.getActionSender().sendSidebarInterface(86, 310);						
					} else {
						player.getActionSender().sendSidebarInterface(86, 319);
					}
					break;
				case ANCIENT_MAGICKS:
					...
					player.getActionSender().sendSidebarInterface(86, 388);
					break;
				case LUNAR_MAGICS:
		...
		case 324:
			...
			if (button >= 148 && button <= 155) {
				amount = 1;
				index = 155 - button;
			} else if (button >= 140 && button <= 147) {
				amount = 5;
				index = 147 - button;
			} else if (button >= 132 && button <= 139) {
				player.getActionSender().removeInterface();
				player.getInterfaceState().openEnterAmountInterface(interfaceId);
				break;
			} else if (button >= 124 && button <= 131) {
				amount = 28;
				index = 131 - button;
			}
			break;
