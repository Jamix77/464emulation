...
import org.hyperion.rs2.clipping.pf.SizedPathFinder;
import org.hyperion.rs2.model.Hit;
import org.hyperion.rs2.model.Mob;
import org.hyperion.rs2.model.NPC;
import org.hyperion.rs2.model.Player;
import org.hyperion.rs2.model.World;
import org.hyperion.rs2.model.CombatNPCDefinition.GodWarsMinion;
...
public class NPCTickTask implements Task {
	...
	public void execute(GameEngine context) {
		...
		if(npc.canMove()) {
			...
			//if(org.osremake07.rs2.model.combat.Region.get(region).checkPath(npc, x, y, false, NpcDef.forId(npc.getDefinition().getId()).size, NpcDef.forId(npc.getDefinition().getId()).size)) {
				//npc.getWalkingQueue().addStep(x, y);
				World.getWorld().doPath(new SizedPathFinder(), npc, x, y);
