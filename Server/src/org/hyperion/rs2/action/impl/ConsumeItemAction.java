...
import org.hyperion.rs2.model.Consumables.Food;
import org.hyperion.rs2.model.Consumables.PotionType;
...
public class ConsumeItemAction extends Action {
	...
	public void execute() {
		...
		try {
			 ...
			 * Food
			 */
			if(getMob().getCombatState().canEat() && !getMob().getCombatState().canDrink()) {
				return;
			}
				...
				case SARADOMIN_BREW:
					...
					for(int i = 0; i < drink.getSkills().length; i++) {
						...
						if(skill == Skills.HITPOINTS) {
							int hitpointsModification = (int) (getMob().getSkills().getLevelForExperience(Skills.HITPOINTS) * 0.17);
							getMob().getSkills().increaseLevelToMaximumModification(skill, hitpointsModification);
						} else if(skill == Skills.DEFENCE) {
							int defenceModification = (int) (getMob().getSkills().getLevelForExperience(Skills.DEFENCE) * 0.22);
							...
							getMob().getSkills().decreaseLevelToMinimum(skill, modification);
						}
					}
					break;
				case ZAMORAK_BREW:
