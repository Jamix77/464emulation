...
public final class NpcDef {
	...
	public static final NpcDef[] npcDefinitions = new NpcDef[Cache
	...
	public static final NpcDef forId(int npcId) {
		if (npcId < 0 || npcId >= npcDefinitions.length)
			return null;
		NpcDef def = npcDefinitions[npcId];
		if (def == null)
			npcDefinitions[npcId] = def = new NpcDef(npcId);
		return def;
	}

	private NpcDef(int id) {
		this.id = id;
